    ...
    <div className="min-h-screen bg-ink-950 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-ink-500 font-ui text-sm mt-1">Welcome back, creator</p>
        </div>
        <form onSubmit={handle} className="bg-ink-800 border border-ink-700 rounded-2xl p-6 space-y-4">
          {[['username','Username','text'],['password','Password','password']].map(([k,l,t]) => (
            <div key={k}>
              <label className="block text-xs font-ui font-semibold text-ink-400 uppercase tracking-wider mb-1.5">{l}</label>
          ...
          <p className="text-center text-sm font-ui text-ink-500">
            No account?{' '}
            <Link to="/register" className="text-amber-400 hover:underline">Join ArtWave</Link>
          </p>
        </form>
      </div>
    </div>
